...
function getCacheKey({ mode, model, year, engine, equipment, hsn, tsn, lang }) {
  const eq = String(equipment || '').trim().toLowerCase();
  const l = String(lang || 'de').trim().toLowerCase();
  if (mode === 'hsn') {
    return `hsn:${String(hsn).trim().toLowerCase()}|${String(tsn).trim().toLowerCase()}|${eq}|${l}`;
  }
  return `model:${String(model).trim().toLowerCase()}|${String(year || '').trim()}|${String(engine || '').trim().toLowerCase()}|${eq}|${l}`;
}
...
const LANGUAGE_NAMES = { de: 'Deutsch', en: 'Englisch', fr: 'Französisch', es: 'Spanisch' };

function buildSystemPrompt(lang) {
  const languageName = LANGUAGE_NAMES[lang] || 'Deutsch';
  return `Du bist ein Assistent für Gebrauchtwagen-Kaufberatung. Nutze Websuche (Foren, Testberichte, DEKRA-Report, TÜV-Report, Fachpresse) um für das angegebene Fahrzeugmodell/Motor konkrete, realistische Informationen zu sammeln.
...
Falls Ausstattungsdetails angegeben sind (z.B. Schaltung, DSG/Automatik, Klimaanlage), berücksichtige diese bei Schwachstellen und Checkliste, sofern dafür bekannte Besonderheiten existieren (z.B. DSG-spezifische Probleme, Klimaanlagen-Kompressor-Verschleiß bei bestimmten Baureihen).

WICHTIG: Schreibe ALLE Textinhalte (Titel, Beschreibungen, Hinweise, year_note) auf ${languageName}. Die JSON-Feldnamen selbst bleiben unverändert wie unten vorgegeben.
...

{
  ...
  "year_valid": true,
  "year_note": "falls year_valid false ist: kurzer Satz auf ${languageName} mit den tatsächlichen Produktionsjahren dieses Modells, sonst leerer String",
  "weak_points": [{"title": "kurzer Titel", "description": "1-2 Sätze auf ${languageName}"}],
  "maintenance": [{"item": "z.B. Ölwechsel", "interval": "z.B. alle 15.000 km / 1 Jahr"}],
  "checklist": ["konkreter Prüfpunkt beim Besichtigen, spezifisch für dieses Modell/diesen Motor, auf ${languageName}"]
}
...
- Maximal 6 Einträge bei weak_points/maintenance/checklist.
...
module.exports = async function handler(req, res) {
  ...
  const { mode, model, year, engine, equipment, hsn, tsn, lang } = req.body || {};
  const safeLang = ['de', 'en', 'fr', 'es'].includes(lang) ? lang : 'de';
  ...
  if (mode === 'hsn') {
    ...
    userRequest = [
      `HSN: ${String(hsn).slice(0, 10)}`,
      `TSN: ${String(tsn).slice(0, 10)}`,
      equipment ? `Ausstattung: ${String(equipment).slice(0, 150)}` : null
    ].filter(Boolean).join('\n');
  } else {
    if (!model) {
  ...
  const cacheKey = getCacheKey({ mode, model, year, engine, equipment, hsn, tsn, lang: safeLang });
  ...
  try {
    ...
    try {
      ...
      console.error('JSON-Parse-Fehler (vermutlich abgeschnittene Antwort):', parseErr.message);
      return res.status(502).json({ error: 'Die Antwort war zu umfangreich und wurde abgeschnitten. Bitte nochmal versuchen.' });
    }
